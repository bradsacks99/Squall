...
class UdpPortScan(ITask):

    datastore = None

    def __init__(self):
        self.uid = "d75d9033-bbfb-4d63-a51d-036bab6a6e62"
        self.name = "UdpPortScan"
        self.desc = "A Squall task to scan UDP ports"
    ...
    async def run(cls, *args, **kwargs) -> None:
        ...
        async def check_port(ip, port, loop):
            conn = asyncio.open_connection(ip, port, loop=loop, proto=socket.SOCK_DGRAM)
            try:
        ...
        subnet = network_data.get('subnet', None)
        ...
        output_data = await scan_entry(ipaddresses, ports, asyncio.get_event_loop(), concurrency)
